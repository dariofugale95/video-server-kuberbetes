ffmpeg -i "$1" \
  ...
  -b:v:0 350k  -c:v:0 libx264 -filter:v:0 "scale=320:-2"  \
  -b:v:1 1000k -c:v:1 libx264 -filter:v:1 "scale=640:-2"  \
  -b:v:2 3000k -c:v:2 libx264 -filter:v:2 "scale=1280:-2" \
  -use_timeline 1 -use_template 1 -window_size 6 -adaptation_sets "id=0,streams=v  id=1,streams=a" \
  -hls_playlist true -f dash "$2"
